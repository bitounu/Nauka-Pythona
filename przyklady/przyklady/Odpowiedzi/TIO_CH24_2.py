...
# Odpowiedź do zadania praktycznego 2, z rozdziału 24.

# Lądownik księżycowy z funkcją "zagraj ponownie"
...
def wyswietl_podsumowanie():
    ...
    podsum5 = u"Chcesz zagrać ponownie? (T lub N)"
    ...
    p5_czcionka = pygame.font.Font(None, 26)
    p5_pow = p5_czcionka.render(podsum5, 1, (255, 255, 255))
    ekran.blit(p5_pow, [20, 210])
    pygame.display.flip()
...
while uruchomiona:
    ...
    if wysokosc > 0.01:
        ...
        zatrzymany = True
        if klawisz == pygame.K_t:  # jeżeli grać nicśnie klawisz 't'
            paliwo = 5000.0       # rozpoczynamy grę od nowa
            predkosc = -100.0
            wysokosc = 2000
            sila_ciagu = 0
        ...
        if zdarzenie.type == pygame.QUIT:
            ...
            if przytrzymany:
                ...
                if mojaSilaCiagu.rect.centery > 500:
                    mojaSilaCiagu.rect.centery = 500
        elif zdarzenie.type  == pygame.KEYDOWN:  # kiedy gra się zakończy, pobieramy nacisnięty klawisz
            if zatrzymany:
