...
# Odpowiedź do zadania praktycznego 1, z rozdziału 24.

# Lądownik księżycowy z funckją wykrywania momentu opuszenia orbity
...
def oblicz_predkosc():
    ...
    # sprawdzamy czy prędkość i wysokość nie są zbyt wysokoe - co znabcza opuszczenie orbity
    if predkosc > 500 or wysokosc > 10000:
        poza_orbita = True
    return(poza_orbita)
...
# Kiedy gra się skończy, wyświetlamy statystyki końcowe
def wyswietl_poza_orbita():
    podsum1 = u"Poza orbitą"
    podsum2 = "Koniec gry"
...
while uruchomiona:
    ...
    fps = zegar.get_fps()
    if fps < 1: fps = 30
    if wysokosc > 0.01 and not poza_orbita:
        poza_orbita = oblicz_predkosc()
        ekran.fill([0, 0, 0])
        wyswietl_statystyki()
