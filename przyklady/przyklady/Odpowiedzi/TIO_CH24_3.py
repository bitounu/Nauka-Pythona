...
# OdpowiedŸ do zadania praktycznego 3, z rozdzia³u 24.

# Wirtualny zwierzak z funkcj¹ pauzy

import sys, pickle,datetime
from PyQt4 import QtCore, QtGui, uic

klasa_formularza, klasa_bazowa = uic.loadUiType("wirtulanyzwierzak_pauza.ui") # u¿ywany UI z przyciskiem pauzy
                                                                                #   i dodatkow¹ etykiet¹
class MojFormularz(klasa_bazowa, klasa_formularza):
    def __init__(self, parent=None):
        ...
        self.btnPauza.clicked.connect(self.btnPauza_klikniety)
        ...
        if plikUchwyt:
            ...
            zapis_lista = [8, 8, 0, datetime.datetime.now(), 0, False]
        ...
        self.pauza = zapis_lista[5] # przywracamy zapisany stan pauzy
        if self.pauza:
            self.btnPauza.setText("Kontunuuj")
            self.lblPauza.setText("Pauza")
        else:
            self.btnPauza.setText("Pauza")
            self.lblPauza.setText("")
...
u"Czy na pewno chcesz obudziæ zwierzaka? Bêdzie bardzo niezadowolony!",
    ...
    def btnPauza_klikniety(self, event):
        self.pauza = not self.pauza
        if self.pauza:
            self.btnPauza.setText("Kontynuuj")
            self.lblPauza.setText("Pauza")
        else:
            self.btnPauza.setText("Pauza")
            self.lblPauza.setText("")
    ...
    def closeEvent(self, zdarzenie):
        ...
        zapis_lista= [self.szczescie, self.zdrowie, self.glod, \
                    datetime.datetime.now(), self.time_cycle, self.pauza] # dodany znacznik pauzy
